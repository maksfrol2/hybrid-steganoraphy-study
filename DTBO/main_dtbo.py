...
class ImageSteganography:
    ...
    def _embed_in_block(self, block, secret_bits, params):
        ...
        params : list
            Parameters [direction, x_offset, y_offset, bit_planes, sb_pole, sb_dire]
    ...
    def _fitness_function(self, particles, block_idx):
        ...
        for i in range(n_particles):
            ...
            # Negative PSNR as we want to maximize PSNR but DTBO minimizes
    ...
    def _run_dtbo(self, SearchAgents, Max_iterations, lowerbound, upperbound, dimension, fitness, initial_population=None):
        """
        Implementation of the Driving Training-Based Optimization (DTBO) algorithm
        ...
        DTBO_curve: numpy.ndarray
            Convergence curve (fitness value in each iteration)
        """
        # Initialize the population randomly within the search space if not provided
        if initial_population is None:
            X = np.zeros((SearchAgents, dimension))
            for i in range(dimension):
                X[:, i] = lowerbound[i] + np.random.random(SearchAgents) * (upperbound[i] - lowerbound[i])
        else:
            X = initial_population.copy()
            
            # Ensure population is within bounds
            for i in range(dimension):
                X[:, i] = np.clip(X[:, i], lowerbound[i], upperbound[i])
        
        # Evaluate the initial population
        fit = np.zeros(SearchAgents)
        for i in range(SearchAgents):
            fit[i] = fitness(X[i, :].reshape(1, -1))
        ...
        for t in range(Max_iterations):
            ...
            idx = np.argsort(fit)
            XF = np.column_stack((X, fit.reshape(-1, 1)))
            XFsort = XF[idx, :]
            X = XFsort[:, :dimension]
            fit = XFsort[:, dimension]
            
            # Select the driving instructors (DI)
            ...
            for i in range(SearchAgents):
                ...
                k_i = np.random.randint(0, N_DI)
                DI_ki = DI[k_i, :].copy()
                F_DI_ki = F_DI[k_i]
                I = round(1 + np.random.random())
                
                if F_DI_ki < fit[i]:
                    X_P1 = X[i, :] + np.random.random() * (DI_ki - I * X[i, :])
                else:
                    X_P1 = X[i, :] + np.random.random() * (X[i, :] - I * DI_ki)
                ...
                X_P2 = PIndex * X[i, :] + (1 - PIndex) * Xbest
                ...
                R = 0.05
                X_P3 = X[i, :] + (1 - 2 * np.random.random(dimension)) * R * (1 - t / Max_iterations) * X[i, :]
        ...
        return Best_pos, Best_score, DTBO_curve
        
    def _optimize_embedding(self):
        """
        Optimize embedding using DTBO for each block
        ...
        for block_idx in range(4):
            ...
            # Initialize population with focus on maximizing capacity
            population = np.zeros((self.n_particles, 6), dtype=np.float64)
            for i in range(self.n_particles):
                ...
                for attempt in range(10):  # Try up to 10 times to find good initial positions
                    ...
                    if capacity >= required_capacity:
                        population[i] = np.array([direction, x_offset, y_offset, bit_planes, sb_pole, sb_dire])
                        break
                
                # If we couldn't find parameters with sufficient capacity, use defaults
                if population[i].sum() == 0 and attempt == 9:
                    # Default to parameters that maximize capacity (typically small offsets)
                    population[i] = np.array([0, 0, 0, 1, 0, 0])
            
            # Define a lambda function that captures block_idx for fitness evaluation
            fitness_function = lambda particles: self._fitness_function(particles, block_idx)
            
            # Run DTBO algorithm
            best_pos, best_score, convergence_curve = self._run_dtbo(
                self.n_particles, 
                self.max_iterations, 
                lb, 
                ub,
...
def main():
    ...
    output_path = "stego_sail_dtbo.jpg"
