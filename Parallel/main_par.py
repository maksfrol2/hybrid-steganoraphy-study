...
class HOA:
    ...
    def optimize(self, init_pos=None):
        ...
        Returns:
        --------
        tuple : (best position, best fitness, final positions)
        """
        ...
        return self.global_best_pos, self.global_best_cost, hikers_pos
...
class ImageSteganography:
    ...
    def _optimize_embedding(self):
        """
        Optimize embedding using sequential PSO → HOA and HOA → PSO for each block,
        selecting parameters based on best PSNR
        """
        optimal_params = []
        min_x = 0
        min_y = 0
        ...
        for block_idx in range(4):
            ...
            # Forward Flow: PSO → HOA
            if self.verbose:
                print(f"\nStarting PSO optimization for Block {block_idx + 1} (Forward Flow)")
            ...
            best_cost_pso, best_pos_pso = optimizer.optimize(cost_function, iters=self.max_iterations_pso,verbose=False)
            
            if self.verbose:
                print(f"PSO optimization completed for Block {block_idx + 1} (Forward Flow)")
                print(f"Switching to HOA optimization for Block {block_idx + 1} (Forward Flow)")
            ...
            best_pos_forward, best_cost_forward, _ = hoa.optimize(init_pos=pso_final_positions)
            
            if self.verbose:
                print(f"HOA optimization completed for Block {block_idx + 1} (Forward Flow)")
            
            # Reversed Flow: HOA → PSO
            if self.verbose:
                print(f"\nStarting HOA optimization for Block {block_idx + 1} (Reversed Flow)")
            ...
            _, _, hoa_final_positions = hoa.optimize()
            
            if self.verbose:
                print(f"HOA optimization completed for Block {block_idx + 1} (Reversed Flow)")
                print(f"Switching to PSO optimization for Block {block_idx + 1} (Reversed Flow)")
            
            # Step 4: Run PSO with HOA's final positions
            optimizer = ps.single.GlobalBestPSO(
                n_particles=self.n_particles,
                dimensions=6,
                options=options,
                bounds=bounds,
                init_pos=hoa_final_positions
            )
            
            best_cost_reverse, best_pos_reverse = optimizer.optimize(cost_function, iters=self.max_iterations_pso)
            
            if self.verbose:
                print(f"PSO optimization completed for Block {block_idx + 1} (Reversed Flow)")
            
            # Evaluate PSNR for both solutions
            _, psnr_forward, _ = self._embed_in_block(self.blocks[block_idx], secret_bits, best_pos_forward)
            _, psnr_reverse, _ = self._embed_in_block(self.blocks[block_idx], secret_bits, best_pos_reverse)
            
            # Select parameters with the best PSNR
            if psnr_forward >= psnr_reverse:
                best_pos = best_pos_forward
                best_cost = best_cost_forward
                best_flow = "PSO → HOA"
            else:
                best_pos = best_pos_reverse
                best_cost = best_cost_reverse
                best_flow = "HOA → PSO"
            ...
            print(f"\nBlock {block_idx+1} optimization complete:")
            print(f"Best fitness: {-best_cost}")
            print(f"PSNR (Forward): {psnr_forward:.2f}, PSNR (Reverse): {psnr_reverse:.2f}")
...
def main():
    ...
    output_path = "stego_sail_parr.jpg"
